...
public class TrainPermissionChecks {
    ...
    public static boolean canBypassTrainTweaks(@NotNull ServerPlayer player) {
        return !isDedicatedServer() || player.hasPermissions(4) ||
                PermissionAPI.getPermission(player, CCPermissionNodes.BYPASS_TRAIN_TWEAKS);
    }

    public static boolean canChangeMotionProfile(@NotNull ServerPlayer player, TrainMotionProfile.Type type) {
        var config = CCConfigs.server().trainTweaksConfig;
        var canBypassTrainTweaks = canBypassTrainTweaks(player);

        if (type == TrainMotionProfile.Type.TOP_SPEED)
            return canBypassTrainTweaks || config.canPlayerChangeMaxSpeed.get();
        else
            return canBypassTrainTweaks || config.canPlayerChangeAcceleration.get();
    }
